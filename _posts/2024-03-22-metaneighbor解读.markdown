...
2. 现在想评估$X^{1}$中细胞类型$i$与$X^{2}$中细胞类型$j$的相似性。将$X^{1}[i]$视为测试集，将$X^{2}$视为训练集，其中细胞类型$j$看作正例，标签为1，其余细胞看作负例，标签为0.也就是$Y^{2}$中第$j$列，预测得到$\hat{Y^{1}}[:,j]$,比较$Y^{1}_{(,i)}$与$\hat{Y^{1}}_{(,j)}$的相似性。
