...
4. 高维。比起scRNA-seq数据，ATAC的维度更高，通常在10万维往上，这是一个简单的MLP可能都会带来几百万的参数量。 
...
多模态的核心是跨模态整合，目前在小数据集上去的了不错的效果，还在推进中。
...
1. tokenization。借助分词器，将文本数据分词，得到token id 序列。
...
2. chunk。将文本数据拼接并截断成等长的序列，构成输入的batch。
...
4. 加载模型并设置训练参数。需要注意的是per_device_train_batch_size 和 gradient_accumulation_steps
